...
class Batch_Loader(object):
    def __init__(self, train_triples, words_indexes, indexes_words, headTailSelector, \
        ...
        #additional code from RotatE
        #print(train_triples)
        self.count = self.count_frequency(train_triples)
    ...
    def __call__(self):
        ...
        if self.neg_ratio > 0:
            ...
            for i in range(last_idx):
                for j in range(self.neg_ratio):
                    ...
                    self.new_triples_values[last_idx + cur_idx] = [-1] #[-1] or [0]

            last_idx += cur_idx + 1
        #print(self.new_triples_indexes)

        #print('hrh: ', hrh) #pair of h,rh
        #print('rc: ', rc)   #pair of rt,t
        #print('trt: ', trt)
        #print('exit function: ',self.new_triples_indexes)

        #print(self.new_triples_indexes[:last_idx, :])
        #print(self.new_triples_values[:last_idx])
        pos_sample_idx = int(last_idx/2)
        head = self.new_triples_indexes[:pos_sample_idx, 0].tolist() #first half is positive sample
        relation = self.new_triples_indexes[:pos_sample_idx, 1].tolist()
        tail = self.new_triples_indexes[:pos_sample_idx, 2].tolist()
        subsampling_weight = []

        for i in range(pos_sample_idx):
            freq = self.count[(head[i],relation[i])]+self.count[(tail[i],-relation[i]-1)]
            freq = math.sqrt(1/freq) if freq > 0 else 0
            subsampling_weight.append(freq)
        ...
        #exit(0)
        return self.new_triples_indexes[:last_idx, :], self.new_triples_values[:last_idx],subsampling_weight
