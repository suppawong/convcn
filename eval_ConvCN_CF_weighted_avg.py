...
with open('CF score/' + args.name + "_fold" + args.fold + '.pkl', 'rb') as f:
    all_CF_score = pickle.load(f)
...
for p in test_papers.keys():
    ...
    #     del score_dict[idx]
        ...
        # print(triplets[i,2],score[i][0])
    # print(score_dict)
    # exit(0)

    # get score from CF here...
    CF_score = all_CF_score[str(p)]
    # print(CF_score)
    # add all cited_paper in CF_score (if not exist)
    # then normailze score in CF_score
    for cited_paper in score_dict.keys():
        if str(cited_paper) not in CF_score:
            CF_score[str(cited_paper)] = 0.0


    CF_score_val = list(CF_score.values())
    CF_score_key = list(CF_score.keys())
    CF_score_normalized_val = (CF_score_val - np.min(CF_score_val)) / np.ptp(CF_score_val) if np.ptp(CF_score_val) != 0 else [0]*len(CF_score_val) # score of CF is normalized here...

    # print(CF_score_normalized_val)

    for i in range(0,len(CF_score_val)):
        CF_score[CF_score_key[i]] = CF_score_normalized_val[i]
    ...
    for cited_paper in score_dict.keys():
        ...
        CF_scr = CF_score.get(str(cited_paper), 0)
        # print(CF_scr)
        # print(ConvCN_scr)
        # exit(0)
        # print(ConvCN_scr,CF_scr)
        # exit(0)
        # if CF_score is None:
        #     CF_scr = 0
        # else:
        #     CF_scr = CF_score.get(str(cited_paper),0)
        combined_score[cited_paper] = alpha*CF_scr + (1-alpha)*ConvCN_scr # weighted average
...
path = os.path.join(os.getcwd(),'result','ConvCN_CF_weighted_avg',args.model_name,'alpha='+str(alpha))
