...
def load_triples_from_txt(filename, words_indexes=None, parse_line=parse_line):
    ...
    for _, line in enumerate(lines):
        ...
        if rel in entities:
            ...
            rel_ind = next_ent
            # print(rel_ind,rel) #just check relation value and index
            next_ent += 1
        ...
        # data[(sub_ind, rel_ind, obj_ind)] = val # for knowledge graph
        data[(int(sub), int(rel), int(obj))] = val
...
def custom_build_data(name='TopicCitation_5_fold',path='./data',fold='1'):
    folder = path + '/' + name + '/'
    train_triples, words_indexes, _ = load_triples_from_txt(folder + 'train_fold'+fold+'.txt', parse_line=parse_line)
    test_triples, words_indexes, indexes_words = load_triples_from_txt(folder + 'test_fold'+fold+'.txt', words_indexes=words_indexes, parse_line=parse_line)

    entity2id, id2entity = read_from_id(folder + '/idpair.txt')
    ...
    with open(folder + 'train_fold'+fold+'.txt') as f:
        lines = f.readlines()
    ...
    left_avg = {}
    # print(left_entity)
    for i in range(1,len(relation2id)+1):
    ...
    return train_triples, test_triples, words_indexes, indexes_words, headTailSelector, entity2id, id2entity, relation2id, id2relation
